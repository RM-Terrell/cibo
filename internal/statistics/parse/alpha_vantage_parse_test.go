...
// Given valid json data with all required keys and values, verify that the parse function returns an expected collection of prices
...
func TestMalformedJsonParseDailyToFloat(t *testing.T) {
	...
	if errorString != "error unmarshaling json: unexpected end of JSON input" {
		t.Errorf(`
				Expected JSON error'
				Got: %v`,
			err)
...
// Given data with a non-numeric closing price and permissive errors mode, verify that the parser returns the valid records only
